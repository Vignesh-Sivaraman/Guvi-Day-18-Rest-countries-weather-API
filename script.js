  ...
  for (countries of data) {
    ...
    async function myFunction() {
      ...
      const res = await fetch(
        `https:api.openweathermap.org/data/2.5/weather?lat=${x[0]}&lon=${x[1]}&appid=fbb352a417c0f980535df5d4a273be35&units=metric`
      );
      const data = await res.json();
      console.log(data);
      ...
      div_e.innerHTML = `Weather: <span>${data?.weather[0].description}</span>`;
      const div_f = document.createElement("div");
      div_f.classList.add("temp");
      div_f.innerHTML = `temp: <span>${data?.main.temp}°C </span>`;
      const div_g = document.createElement("div");
      div_g.classList.add("wind_speed");
      div_g.innerHTML = `wind speed: <span>${data?.wind.speed}m/sec </span>`;
...
// const press = document.getElementsByClassName("btns");
// console.log(press.textContent);
